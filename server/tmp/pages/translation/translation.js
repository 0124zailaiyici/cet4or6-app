...
function getEmoji(chinese) { return EMOJIS[chinese] || '📝'; }
function getTopic(chinese) { return TOPICS[chinese] || '其他'; }
function getTag(item) {
    if (item.chinese.length > 80)
        return { type: 'para', label: '段落' };
    if (item.source.includes('真题'))
        return { type: 'exam', label: '真题' };
    return { type: 'sim', label: '模拟' };
}
function genSuggestions(s) {
    if (s.total >= 90)
        return '太棒啦，翻译很准确，继续保持～ 🌟';
    if (s.total >= 80)
        return '不错哦，整体很好，试试用更丰富的表达吧 ✨';
    const parts = [];
    if (s.dimensions.vocabulary < 60)
        parts.push('关键词有遗漏，注意覆盖核心词汇');
    if (s.dimensions.grammar < 60)
        parts.push('句式可以调整一下');
    if (s.dimensions.semantics < 60)
        parts.push('意思表达不够准确');
    if (s.dimensions.expression < 60)
        parts.push('表达可以更地道');
    if (parts.length === 0)
        return '基本正确，再打磨一下就完美啦 💪';
    return parts.join('；');
}
/* 逐词对照 */
function wordCompare(user, ref) {
...
Page({
    data: {
        page: 'dashboard', step: 'list', translations: [], currentItem: null,
        userAnswer: '', result: null,
        history: [], questionHistory: [], completedIds: [],
    ...
    loadData() {
        ...
        const listItems = items.map(t => {
            ...
            return Object.assign(Object.assign({}, t), { _display: t.chinese.length > 40 ? t.chinese.slice(0, 40) + '…' : t.chinese, _emoji: getEmoji(t.chinese), _tag: getTag(t), _level: lv, _topic: getTopic(t.chinese), _done: doneSet.has(t.id), _fav: favSet.has(t.id), _hint: HINTS[lv - 1] || HINTS[0] });
        });
        ...
        // 弱项分析
        const dimNames = { vocabulary: '词汇', grammar: '语法', semantics: '语义', expression: '表达' };
        const dimScores = {};
    ...
    switchPage(e) {
        ...
        if (p === 'practice' && !this.data.currentItem) {
            wx.showToast({ title: '请先从列表选一题', icon: 'none' });
            return;
        }
    ...
    nextQ() {
        ...
        if (!currentItem)
            return;
        const idx = translations.findIndex((t) => t.id === currentItem.id);
        const lv = currentItem._level || 1;
        /* 找同等级下一题 */
        let next = translations.slice(idx + 1).find((t) => t._level === lv && !t._done);
        /* 没有未完成的同等级题，找下一等级 */
        if (!next)
            next = translations.slice(idx + 1).find((t) => !t._done);
        /* 兜底：从头找第一个未完成 */
        if (!next)
            next = translations.find((t) => !t._done);
        /* 全部完成 */
        if (!next) {
