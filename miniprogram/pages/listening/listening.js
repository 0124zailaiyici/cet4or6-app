...
const listening_generated_1 = __importDefault(require("../../data/listening_generated"));
...
function sectionLabel(q) {
    if (q >= 1 && q <= 7)
        return LABELS.sectionA;
    if (q >= 8 && q <= 15)
        return LABELS.sectionB;
    if (q >= 16 && q <= 25)
...
function buildPages(passage) {
    ...
    function pushDir() {
        if (dirs.length > 0) {
            ...
            if (txt.length > 5)
                pages.push({ type: 'dir', section: 'Directions', stem: txt });
            dirs.length = 0;
        }
    }
    function finalizeQ() {
    ...
    function addOpts(text) {
        ...
        for (const p of parts) {
            ...
            if (m) {
                ...
                if (!existing)
                    currentQ.opts.push({ l: m[1], t: m[2].trim() });
            }
        }
    }
    for (const line of lines) {
        if (/^\d{4}年\d+月/.test(line.trim()) && /第\d+页/.test(line))
            continue;
...
Page({
    ...
    onLoad(options) {
        if (options && options.examMode === '1')
            this.setData({ examMode: true });
        audio.attach(this);
        const passages = listening_generated_1.default;
        const app = getApp();
        ...
        if (options.passageId) {
            ...
            if (passage) {
                this.enterDetail({ currentTarget: { dataset: { id: passage.id } } });
            }
        }
    },
    onShow() {
    ...
    viewSummary() {
        ...
        let correctCount = 0;
        const results = this.data.pages.map((p, pi) => {
            if (p.type !== 'q')
                return null;
    ...
    seekAudio(e) {
        ...
        query.select('.seek-track').boundingClientRect((rect) => {
            if (!rect || !rect.width)
                return;
        ...
        if (this.data.focusMode) {
            ...
            if (passage) {
                if (passage.audioUrl)
                    ...
                    this.playText(passage.sentences[origIndex].text);
            }
        }
    },
    prevSentence() {
