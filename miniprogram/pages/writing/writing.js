...
function scoreSentenceLocal(text, pattern) {
    ...
    parts.push('');
    if (wc < 3)
        parts.push('⚠️ 句子太短，建议至少 5 词');
    else if (wc > 40)
        parts.push('⚠️ 句子偏长，建议 15-25 词');
    else
        parts.push('✅ 长度适中');
    ...
    if (text.toLowerCase().includes(cleanPat))
        parts.push('✅ 句型使用正确');
    else
        parts.push('💡 未检测到句型，试试：' + pattern);
    parts.push('');
...
Page({
    ...
    onLoad() {
        const app = getApp();
        const recent = wx.getStorageSync('writingRecentPatterns') || [];
        const pats = sentence_patterns_1.default;
        const writes = writings_1.default;
    ...
    computePatternVisible(category, search) {
        ...
        return this.data.patterns.map(p => {
            if (category !== '全部' && (this.data.patternCategories[p.id] || '通用') !== category)
                return false;
            if (q && p.pattern.toLowerCase().indexOf(q) === -1 && p.chinese.indexOf(q) === -1)
                return false;
            return true;
        });
    },
            ...
            if (this.data.aiAvailable && this.data.aiEnabled) {
                try {
                    const res = yield Promise.race([
                        (0, api_1.teachSentence)(pattern.pattern, text),
    ...
    submitWriting() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.data.aiAvailable && this.data.aiEnabled) {
                try {
                    const ai = yield Promise.race([
                        (0, api_1.correctWriting)(prompt, text),
                        new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 8000)),
                    ]);
                    if (ai.dimensions)
                        dimensions = ai.dimensions;
