    ...
    try {
        ...
        if (type === 'correct') {
            ...
            for (let i = 0; i < 2; i++) {
                ...
                g.gain.value = 0.2;
                g.gain.exponentialRampToValueAtTime(0.01, now + 0.15 + i * 0.12);
                o.start(now + i * 0.12);
                o.stop(now + 0.2 + i * 0.12);
            }
        }
        else if (type === 'wrong') {
            const o = ctx.createOscillator();
            ...
            [523, 587, 659, 784].forEach((f, i) => {
                ...
                o.frequency.value = f;
                g.gain.value = 0.2;
                g.gain.exponentialRampToValueAtTime(0.01, now + 0.15 + i * 0.15);
                o.start(now + i * 0.15);
                o.stop(now + 0.2 + i * 0.15);
            });
        }
        else if (type === 'flip') {
            const o = ctx.createOscillator();
...
function extractWords() {
    const ver = wx.getStorageSync(EXTRACTED_VER_KEY);
    if (ver !== EXTRACTED_VER) {
        wx.removeStorageSync(EXTRACTED_CACHE_KEY);
    }
    const cached = wx.getStorageSync(EXTRACTED_CACHE_KEY);
    if (cached && cached.length)
        return cached;
    const wordMap = {};
    for (const r of readings_1.default) {
        ...
        for (const t of tokens) {
            ...
            if (!known)
                continue;
            if (wordMap[t]) {
                if (wordMap[t].source.indexOf(source) === -1)
                    wordMap[t].source += ' / ' + source;
                continue;
            }
...
function hasCJK(s) { return !!(s && /[\u4e00-\u9fff]/.test(s)); }
let _swipeStartX = 0;
Page({
    data: {
        words: [],
    ...
    onShow() {
        ...
        const packStats = PACKS.map(p => wx.getStorageSync('pack_' + p.id) || 0);
        const achProgress = ACHS.map(a => wx.getStorageSync(a.key) || 0);
        const savedSents = wx.getStorageSync('saved_sentences') || [];
        const sentFontSize = wx.getStorageSync('sent_font_size') || 16;
        this.setData({ darkMode: (0, theme_1.getDarkMode)(), mode, theme, challengeDone, challengeStreak, packStats, achProgress, savedSents, sentFontSize });
    ...
    loadWords(skipTranslate) {
        ...
        if (!stored || stored.length === 0 || ver !== EXTRACTED_VER) {
            ...
            if (stored && stored.length > 0) {
                for (const f of fresh) {
                    ...
                    if (old) {
                        Object.assign(f, { status: old.status, correctStreak: old.correctStreak, chn: old.chn || f.chn, contextCn: old.contextCn || f.contextCn });
                    }
                }
                const manual = stored.filter(v => v.source === '手动添加' || !fresh.find(f => f.word === v.word));
                for (const m of manual) {
                    if (!fresh.find(f => f.word === m.word))
                        fresh.push(m);
                }
            }
            stored = fresh;
        ...
        for (const w of words) {
            ...
            if (tab === 0 && w.status !== 'master')
                filtered.push(w);
            else if (tab === 1 && w.status === 'review')
                filtered.push(w);
            else if (tab === 2 && w.status === 'master')
                filtered.push(w);
        }
        this.setData(Object.assign(Object.assign({ words, filteredWords: filtered }, stats), { streak: this.data.streak, loading: false }));
        if (!skipTranslate)
    ...
    fetchChinese(w) {
        return __awaiter(this, void 0, void 0, function* () {
            if (w.chn)
                return true;
            try {
                const result = yield (0, api_1.lookupWord)(w.word);
            ...
            try {
                const ai = yield (0, api_1.aiTranslateWord)(w.word);
                if (ai.chinese) {
                    w.chn = ai.chinese;
                    return true;
                }
    ...
    persistChn(w) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!w.chn)
                return;
            const app = getApp();
            const stored = app.globalData.studyData.vocabWords;
    ...
    fetchContextCn(w) {
        return __awaiter(this, void 0, void 0, function* () {
            if (!w.context || w.contextCn)
                return;
            try {
                const r = yield (0, api_1.translateText)(w.context);
    ...
    preTranslateContexts() {
        return __awaiter(this, void 0, void 0, function* () {
            const words = this.data.words;
            if (!words || words.length === 0)
                return;
            const tver = wx.getStorageSync(TRANSLATION_VER_KEY);
            ...
            for (const w of needTrans)
                ctxSet.add(w.context);
            const uniqueCtxs = [...ctxSet];
            try {
                const result = yield (0, api_1.translateTextBatch)(uniqueCtxs);
                if (result && result.results && result.results.length === uniqueCtxs.length) {
                    const ctxMap = new Map();
                    for (let i = 0; i < uniqueCtxs.length; i++)
                        ctxMap.set(uniqueCtxs[i], result.results[i]);
                    for (const w of needTrans) {
    ...
    showGameForIdx(idx, skipFrom) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (!word.chn && hasCJK(word.definition)) {
                word.chn = word.definition;
                word.definition = '';
            }
            if (!word.chn) {
                this.setData({ gameLoading: true, gameWord: null });
                const ok = yield this.fetchChinese(word);
                if (ok)
                    yield this.persistChn(word);
                if (!word.chn) {
            ...
            if (!word.contextCn && word.context)
                this.fetchContextCn(word);
            if (!word.audioUrl)
                this._fetchAudioUrl(word);
        });
    },
    flipCard() {
        playSfx('flip');
    ...
    markKnown() {
        ...
        const app = getApp();
        const sw = app.globalData.studyData.vocabWords.find(v => v.word === w.word);
        const newStreak = sw ? sw.correctStreak + 1 : 1;
        const newStatus = newStreak >= 3 ? 'master' : 'learning';
    ...
    openStoryView() {
        const unlocked = wx.getStorageSync('unlocked_stories') || 0;
    ...
    selectPack(e) {
        ...
        if (!id) {
            this.setData({ selectedPack: '' });
            return;
        }
        const p = PACKS.find(x => x.id === id);
        if (!p)
            return;
        const got = wx.getStorageSync('pack_' + id) || 0;
    ...
    _completeChallenge() {
        const today = new Date().toDateString();
        const streak = (wx.getStorageSync('challenge_streak') || 0) + 1;
        wx.setStorageSync('challenge_date', today);
        wx.setStorageSync('challenge_streak', streak);
        const prevWeek = wx.getStorageSync('ach_week') || 0;
        if (streak > prevWeek)
            wx.setStorageSync('ach_week', streak);
    ...
    _checkPackCompletion(word) {
        ...
        const completePacks = PACKS.filter(p => (wx.getStorageSync('pack_' + p.id) || 0) >= p.words.length).length;
    ...
    _hardRefresh() {
        const app = getApp();
        const raw = app.globalData.studyData.vocabWords;
        const tab = this.data.tab;
        const sq = (this.data.searchQuery || '').toLowerCase();
        const words = raw.map(w => (Object.assign({}, w)));
        const filtered = [];
        for (const w of words) {
            let pass = true;
            if (sq)
    ...
    _fetchAudioUrl(word) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const result = yield (0, api_1.lookupWord)(word.word);
                const entry = Array.isArray(result) ? result[0] : result;
                const au = entry && entry.phonetics && (entry.phonetics.find((p) => p && p.audio) || {}).audio;
                if (au) {
                    word.audioUrl = au;
                    const app = getApp();
                    wx.setStorageSync('studyData', app.globalData.studyData);
    ...
    startConfetti() {
        ...
        const modeColors = { battle: ['#ff3d00', '#d50000', '#ff6d00', '#ff9100'], garden: ['#66bb6a', '#43a047', '#81c784', '#a5d6a7'], card: ['#ab47bc', '#8e24aa', '#ce93d8', '#e1bee7'] };
        ...
        wx.createSelectorQuery().select('#confettiCanvas').fields({ node: true, size: true }).exec((res) => {
            ...
            const draw = () => {
                ...
                if (alive)
                    anim = canvas.requestAnimationFrame && canvas.requestAnimationFrame(draw);
            };
            draw();
            canvas._confettiAnim = anim;
        });
    },
    stopConfetti() {
        wx.createSelectorQuery().select('#confettiCanvas').fields({ node: true }).exec((res) => {
            const canvas = res[0] && res[0].node;
            if (canvas && canvas._confettiAnim != null)
                canvas.cancelAnimationFrame && canvas.cancelAnimationFrame(canvas._confettiAnim);
    ...
    _saveAddedWord(w, phonetic, chn, context, contextCn, words) {
        const newWord = {
    ...
    _doAddWord(w) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                let dict404 = false;
                try {
                    const result = yield (0, api_1.lookupWord)(w);
                    const entry = Array.isArray(result) ? result[0] : result;
                    if (entry && entry.chinese)
                        chn = entry.chinese;
                    if (entry && entry.phonetic)
                        phonetic = entry.phonetic;
                    else if (entry && entry.phonetics && entry.phonetics[0] && entry.phonetics[0].text)
                ...
                if (!chn && !dict404) {
                    try {
                        const ai = yield (0, api_1.aiTranslateWord)(w);
                        if (ai && ai.chinese)
                            chn = ai.chinese;
                    }
                    catch (_a) { }
                ...
                let contextCn = '';
                try {
                    const sents = yield (0, api_1.generateSentence)({ word: w, count: 1 });
