...
const reading_annotations_1 = __importDefault(require("./reading_annotations"));
...
Page({
    ...
    onLoad(options) {
        ...
        const readings = readings_1.default;
    ...
    select(e) {
        ...
        if (item) {
            const annot = reading_annotations_1.default[item.id];
    ...
    onWordTap(e) {
        ...
        const word = (ds.word || '').toLowerCase().trim();
        const zh = ds.zh || '';
    ...
    getPassageParas(text) {
        ...
        for (const s of sentences) {
            ...
            currentPara += (currentPara ? '. ' : '') + s.trim();
            count++;
        }
        if (currentPara)
            paras.push(currentPara.trim() + '.');
    ...
    jumpToParagraph(e) {
        const locate = e.currentTarget.dataset.locate;
        if (!locate)
            return;
        const st = this.data.current && this.data.current.sectionType;
        let paraIdx = -1;
    ...
    onChoiceTap(e) {
        if (this.data.submitted)
            return;
        const choice = e.currentTarget.dataset.letter;
        if (!choice)
            return;
    ...
    updateCompact() {
        ...
        if (!choices || choices.length < 4) {
            this.setData({ compactOpts: false });
            return;
        }
        const allShort = choices.every((c) => c.length < 25);
    ...
    onQuestionTouchEnd(e) {
        ...
        if (this.data.current && this.data.current.sectionType === 'B') {
            if (dx > 50 && this.data.bStmtPage > 0)
                this.setData({ bStmtPage: this.data.bStmtPage - 1 });
            else if (dx < -50 && this.data.bStmtPage < this.data.bStmtPages.length - 1)
                this.setData({ bStmtPage: this.data.bStmtPage + 1 });
        }
        else {
            if (dx > 50)
